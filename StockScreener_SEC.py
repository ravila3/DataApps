...
def update_yahoo_data_for_company(cik):
    ...
    try:
        ...
        div_yield = clean_number(yahoo_stats.get("dividendYield")) or 0.0
        ...
        'div_yield' : safe_round(div_yield,1),
...
def display_stock_analysis_form(stock_growth_analysis_df):
    ...
    columns = [ 'cik', 'ticker', 'company_and_ticker','industry','sector'] + editable_columns + ['curr_quantity','stock_price','curr_value','gain_pct', # 'price_range_52wks',
        ...
        'Last3Q_Revenue_Growth_PCT', 'Last3Q_Income_Growth_PCT', 'Last3Q_Median_Margin_PCT', 'Last3Q_Margin_Growth_PCT', 'Last3Q_Income_Positive',
    ...
    st.data_editor(styled, key="my_editor", on_change=on_change_handle, width='stretch', disabled=disabled_cols
                ...
                "notes": st.column_config.TextColumn(label="Notes", help="Editable notes for this stock", pinned=False, width="medium"),
                'Pct_Chg_from_52_Wk_High':st.column_config.NumberColumn(label="% from 52wk High", help="Percentage change from 52-week high", format='%.1f', width="small"),
                'Pct_Chg_from_52_Wk_Low':st.column_config.NumberColumn(label="% from 52wk Low", help="Percentage change from 52-week low", format='%.1f', width="small"),
                'Pct_Chg_from_7_Days_Ago':st.column_config.NumberColumn(label="% from 7 days ago", help="Stock Price change from 7 days ago", format='%.1f', width="small"),
                'trailing_pe':st.column_config.NumberColumn(label="P/E Trailing", help="Trailing Price to Earnings Ratio", format='%.1f', width="small"),
                'forward_pe':st.column_config.NumberColumn(label="P/E Fwd", help="Forward Price to Earnings Ratio based on analyst estimated earnings", format='%.1f', width="small"),
                'trailing_ps':st.column_config.NumberColumn(label="P/S Trailing", help="Trailing Price-to-Sales Ratio", format='%.1f', width="small"),
                'div_yield':st.column_config.NumberColumn(label="Div Yield", help="Dividend Yield", format='%.1f', width="small"),
                "Consolidated_Score": st.column_config.NumberColumn(label="Consol Value Score", help="Consolidated Value Score", format='%.1f', width="small"),
                'Growth_Quality': st.column_config.NumberColumn(label="Growth_Quality", help="Growth Quality Score", format='%.1f', width="small"),
                'Recent_Momentum': st.column_config.NumberColumn(label="Recent_Momentum", help="Recent Momentum Score", format='%.1f', width="small"),
                'Stability_Trend':  st.column_config.NumberColumn(label="Stability_Trend", help="Stability Trend Score", format='%.1f', width="small"),
                'Value_Pressure':  st.column_config.NumberColumn(label="Value_Pressure", help="Value Pressure Score", format='%.1f', width="small"),
                "Last3Q_Revenue_Growth_PCT": st.column_config.NumberColumn(label="Rev Growth % - Last 3Q",help="Revenue Growth % - Last 3Q", format='%,.1f', width="small"),
                "Last3Q_Income_Growth_PCT": st.column_config.NumberColumn(label="Inc Growth % - Last 3Q",help="Income Growth % - Last 3Q", format='%,.1f', width="small"),
                "Last3Q_Median_Margin_PCT": st.column_config.NumberColumn(label="Median Margin % - Last 3Q",help="Median Margin % - Last 3Q", format='%,.1f', width="small"),
                "Last3Q_Margin_Growth_PCT": st.column_config.NumberColumn(label="Margin Growth % - Last 3Q",help="Margin Growth % - Last 3Q", format='%,.1f', width="small"),
                ...
                'Revenue_Growth_Median':st.column_config.NumberColumn(label="Revenue Median", format='dollar', step='int', width="small"),
                },
                )
